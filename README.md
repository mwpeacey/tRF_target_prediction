...
5. Second pass alignment with "wrappers/STAR_wrapper.sh", using the updated STAR index from step 1.4.
...
7. Merge stringtie assembly into a single GTF file and filter for strand information with "transcriptome_assembly/stringtie_merge.sh". This requires moving all .gtf files from step 1.6 into a single directory. 
8. Extract fasta sequences from the transcriptome GTF generated in step 1.7 with "transcriptome_assembly/gtf_to_fasta.sh". Individual transcripts are split into separate fasta files for faster miRanda processing.
