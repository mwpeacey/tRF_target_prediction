...
## real scan (e.g. *_10000bp_windows.fa). Only the plus windows are needed;
## minus-strand targets are derived by reverse-complement. Sampling is restricted
## to the canonical chromosomes (hardcoded in sample_windows_from_fasta.py).
##
...
if [ "$#" -lt 8 ]; then
  echo "Usage: $0 <scripts_dir> <windows_plus.fa> <sRNA.fa> <out_dir> <run_mode> <shuffle_target> <score_threshold> <n_iterations> [fraction] [seed] [qos]" >&2
  exit 1
fi
...
SETUP_JOB=$(sbatch --parsable \
  ...
  "$SCRIPTS" "$WINDOWS_PLUS" "$SRNA_FA" "$OUTDIR" "$RUN_MODE" "$SCORE_THRESHOLD" "$FRACTION" "$SEED"
)
