...
##          <out_dir> <run_mode> <score_threshold> <n_iterations> \
##          [fraction] [seed]
##
## Example (production — threshold 80, 1000 iterations, 20% of genome):
...
if [ "$#" -lt 7 ]; then
  echo "Usage: $0 <scripts_dir> <genome.fa> <sRNA.fa> <out_dir> <run_mode> <score_threshold> <n_iterations> [fraction] [seed]" >&2
  exit 1
fi
...
ITER_JOB=$(sbatch --parsable \
  ...
  "$SCRIPTS" "$SRNA_FA" "$OUTDIR" "$RUN_MODE" "$SCORE_THRESHOLD"
)
