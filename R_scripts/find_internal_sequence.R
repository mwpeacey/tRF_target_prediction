...
find_downstream_internal_sequence <- function(LTR_df, LTR_name, max_gap = 200) {
  
  # 1) Split: selected LTR vs internal hits
  LTR_query <- LTR_df[grepl(LTR_name, LTR_df$repName, ignore.case = FALSE), ]
  LTR_int   <- LTR_df[grepl("int", LTR_df$repName, ignore.case = TRUE), ]
  ...
  # 3) Downstream windows (strand-aware)
  sub_plus  <- subject[strand(subject) == "+"]
  sub_minus <- subject[strand(subject) == "-"]
  
  win_plus <- GRanges(
    seqnames = seqnames(sub_plus),
    ranges   = IRanges(start = end(sub_plus) + 1,
                       end   = end(sub_plus) + max_gap),
    strand   = strand(sub_plus)
  )
  
  win_minus <- GRanges(
    seqnames = seqnames(sub_minus),
    ranges   = IRanges(start = pmax(start(sub_minus) - max_gap, 1),
                       end   = pmax(start(sub_minus) - 1, 0)),
    strand   = strand(sub_minus)
  )
  
  # Keep LTR indices
  win_plus$ltr_idx  <- seq_along(sub_plus)
  win_minus$ltr_idx <- seq_along(sub_minus)
  ...
  # 5) Distances
  df_plus <- as.data.frame(hits_plus) %>%
    mutate(ltr_idx = win_plus$ltr_idx[subjectHits],
           dist_bp = start(query[queryHits]) - end(sub_plus)[ltr_idx])
  
  df_minus <- as.data.frame(hits_minus) %>%
    mutate(ltr_idx = win_minus$ltr_idx[subjectHits],
           dist_bp = start(sub_minus)[ltr_idx] - end(query[queryHits]))
  
  # 6) Nearest downstream internal sequence per LTR
  pick_nearest <- function(df) {
    if (nrow(df) == 0) return(df)
    df %>% group_by(ltr_idx) %>%
      slice_min(order_by = dist_bp, with_ties = FALSE) %>%
  ...
  # 7) Counts
  if (nrow(df_near) > 0) {
    nearest_internal_rep <- mcols(query)$repName[df_near$queryHits]
    counts <- as.data.frame(table(nearest_internal_rep), stringsAsFactors = FALSE) %>%
      dplyr::arrange(dplyr::desc(Freq)) %>%
      dplyr::rename(internal_repName = nearest_internal_rep, n_downstream_hits = Freq)
  } else {
    counts <- tibble(internal_repName = character(), n_downstream_hits = integer())
  }
...
unique(LTR[grepl("VL30", LTR$repName, ignore.case = TRUE), ]$repName)

find_internal_sequence(LTR, "MMVL30-int")

find_internal_sequence(LTR, "MERVL-int")
